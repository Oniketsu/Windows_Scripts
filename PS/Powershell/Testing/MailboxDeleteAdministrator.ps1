...
$Mailbox = "yitzy@useinc.com"

#--DELETE MAIL--
#Foreach ($Mailbox in $Mailboxes) {
    Write-Host ("Deleting all mail received before " + $EndDate + " from " + $Mailbox + "...") -ForegroundColor Green
    Try {
        Do {
            $Result = (Get-Mailbox -identity $Mailbox | Search-Mailbox -SearchQuery ('Received<="' + $EndDate + '" OR Sent<="' + $EndDate + '"') -DeleteContent -Force)
        } Until ($Result.ResultItemsCount -eq 0)
        Write-Host ("Mail deleted from " + $Mailbox + ".") -ForegroundColor Green
    }
    Catch {

...
Try {
    Foreach ($Drive in $Disks) {
        $TempFolders += Get-Childitem -Path ($Drive.DeviceID + "\") -Directory -Include *QBBackupTemp* -Recurse -ErrorAction SilentlyContinue | Where{$_.LastWriteTime -le (Get-Date).AddDays(-15)}
    }

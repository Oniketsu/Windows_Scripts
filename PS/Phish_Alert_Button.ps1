...
#--Button Info----;
$InstallerName = "Phish_Alert.msi"
$DebugLog = Join-Path $Env:TMP PABDebug.log
$MsiLog = Join-Path $Env:TMP PABInstall.log
...
function Create-Dir{
    ...
    If ((gwmi -Class Win32_ComputerSystem).SystemType -match '(x64)') {
        $FileURL = ($FTP + "/Phish_Alert.msi")
        $Output = "$Directory\Phish_Alert.msi";
...
function main{
    ...
    If (!(Get-Service "Phish Alert Button" -ErrorAction SilentlyContinue)) {
        Create-Dir
       Download-MSI
      Install-PAB
